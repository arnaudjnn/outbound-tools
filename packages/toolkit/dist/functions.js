...
import { fetchEmails, fetchSentEmails, appendToSent, setEmailFlag, removeEmailFlag, matchRepliesToSent, filterByTagExpression, resolveFolder, countByKeyword, addAudienceSegments, removeAudienceSegments, fetchEmailByUid, fetchEmailRawByUid, deleteEmail, getEmailHeaders, fetchDrafts, saveDraft, deleteDraft, fetchAttachmentByUid, normalizeSubject, extractEmail, resolveDraftsFolder, saveCampaignConfig, loadCampaignConfig, listCampaignConfigs, deleteCampaignConfig, upsertContactMarker, removeContactMarkerSegments, listAudienceSegmentsWithContacts, getContactMetadataByEmails, } from "./imap.js";
...
export async function list_received_emails(params) {
...
export async function list_sent_emails(params) {
    const { email, limit, page, tag_filter } = params;
    const mailbox = await getMailboxByEmail(email);
...
export async function list_threads(params) {
...
export async function get_email_account_analytics(params) {
    const { email } = params;
    const mailbox = await getMailboxByEmail(email);
    ...
    const counts = await Promise.all(statusTags.map((tag) => countByKeyword(mailbox, sentFolder, tag)));
    const statuses = {};
    const rates = {};
    const rate = (count) => Math.round((count / totalSent) * 10000) / 100;
...
export async function add_email_tag(params) {
...
export async function remove_email_tag(params) {
    const { email, uid, tag, folder } = params;
    const mailbox = await getMailboxByEmail(email);
    const resolvedFolder = await resolveFolder(mailbox, folder);
...
export async function add_to_audience(params) {
...
export async function remove_from_audience(params) {
    ...
    for (const mb of mailboxes) {
        ...
        for (const folder of ["INBOX", "SENT"]) {
            ...
            for (const seg of segments) {
                if (!mergedSegments.has(seg.name))
                    mergedSegments.set(seg.name, new Set());
...
export async function get_email(params) {
    const { email, uid, folder } = params;
    const mailbox = await getMailboxByEmail(email);
    const resolvedFolder = await resolveFolder(mailbox, folder);
    return fetchEmailByUid(mailbox, resolvedFolder, uid);
}
export async function get_email_raw(params) {
    const { email, uid, folder } = params;
    const mailbox = await getMailboxByEmail(email);
    const resolvedFolder = await resolveFolder(mailbox, folder);
...
export async function reply_to_email(params) {
...
export async function reply_all_to_email(params) {
    const { email, uid, folder, text, html } = params;
    if (!text && !html) {
        throw new Error("At least one of `text` or `html` body is required.");
    }
...
export async function forward_email(params) {
...
export async function delete_email_fn(params) {
    const { email, uid, folder } = params;
    const mailbox = await getMailboxByEmail(email);
    const resolvedFolder = await resolveFolder(mailbox, folder);
...
export async function get_thread(params) {
...
export async function get_attachment(params) {
...
export async function list_drafts(params) {
    const { email, limit, page } = params;
    const mailbox = await getMailboxByEmail(email);
    return fetchDrafts(mailbox, limit, page);
}
export async function get_draft(params) {
...
export async function create_draft(params) {
...
export async function update_draft(params) {
...
export async function delete_draft_fn(params) {
    const { email, uid } = params;
    const mailbox = await getMailboxByEmail(email);
    await deleteDraft(mailbox, uid);
    return { message: `Draft UID ${uid} deleted.` };
}
export async function send_draft(params) {
    const { email, uid } = params;
    const mailbox = await getMailboxByEmail(email);
...
export async function set_reply_status(params) {
...
export async function get_campaign_analytics(params) {
    ...
    for (const e of campaignSent) {
        ...
        if (!steps.has(stepName))
            steps.set(stepName, { sent: 0, variants: new Map() });
    ...
    for (const e of campaignSent) {
        ...
        if (!variantStatuses[variantFlag])
            variantStatuses[variantFlag] = { sent: 0 };
    ...
    return {
        ...
        statusRates: Object.fromEntries(Object.entries(statusCounts).map(([k, v]) => [k, rate(v)])),
        steps: stepAnalytics,
        variants: variantStatuses,
    };
}
export async function create_campaign(params) {
...
export async function list_campaigns(params) {
    const { email } = params;
    const mailbox = await getMailboxByEmail(email);
    const campaigns = await listCampaignConfigs(mailbox);
    return { campaigns };
}
export async function get_campaign(params) {
    const { email, campaign } = params;
    const mailbox = await getMailboxByEmail(email);
    return loadCampaignConfig(mailbox, campaign);
}
export async function delete_campaign_fn(params) {
    const { email, campaign } = params;
    const mailbox = await getMailboxByEmail(email);
    await deleteCampaignConfig(mailbox, campaign);
    return { message: `Campaign "${campaign}" deleted.` };
}
export async function start_campaign(params) {
    const { email, campaign } = params;
    const mailbox = await getMailboxByEmail(email);
    ...
    for (const mb of mailboxes) {
        ...
        for (const folder of ["INBOX", "SENT"]) {
            ...
            if (target) {
                for (const c of target.contacts)
    ...
    function interpolate(template, contactEmail) {
    ...
    for (const contactEmail of contacts) {
        ...
        const contactReplies = inboxPage.emails.filter((e) => extractEmail(e.from) === contactEmail);
        const hasTerminalStatus = contactReplies.some((e) => e.flags.some((f) => TERMINAL_STATUSES.includes(f)));
        ...
        const contactSent = campaignSent.filter((e) => e.to.toLowerCase().includes(contactEmail));
        ...
        try {
            ...
            const justSent = recentSent.emails.find((e) => e.to.toLowerCase().includes(contactEmail));
            if (justSent) {
                const sentFolder = await resolveFolder(mailbox, "SENT");
