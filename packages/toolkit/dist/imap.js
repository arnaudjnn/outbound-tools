...
export function matchRepliesToSent(received, sent) {
    const matches = [];
    const unmatchedUids = [];
...
export async function findSentFolder(client, host) {
    try {
...
function tokenizeTagFilter(input) {
    const tokens = [];
    let i = 0;
...
function parseTagFilter(input) {
    ...
    function parseAtom() {
        if (pos >= tokens.length)
            throw new Error("Unexpected end of tag filter expression");
        if (tokens[pos] === "(") {
            pos++;
            const expr = parseOr();
            if (pos >= tokens.length || tokens[pos] !== ")")
                throw new Error("Missing closing parenthesis in tag filter");
            pos++;
            return expr;
        }
    ...
    if (pos < tokens.length)
        throw new Error(`Unexpected token "${tokens[pos]}" in tag filter`);
    return result;
}
function evalTagFilter(expr, flags) {
    switch (expr.type) {
        case "tag":
...
export function filterByTagExpression(emails, filter) {
...
export async function resolveFolder(mailbox, folder) {
...
export async function listFolders(mailbox) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function fetchEmails(mailbox, folder, limit, page = 1) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function appendToSent(mailbox, raw) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function fetchSentEmails(mailbox, limit, page = 1) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const sentFolder = await findSentFolder(client, mailbox.imapHost);
...
export async function setEmailFlag(mailbox, folder, uid, flag) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function removeEmailFlag(mailbox, folder, uid, flag) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function countByKeyword(mailbox, folder, keyword) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function addAudienceSegments(mailbox, folder, contactEmail, segments) {
...
export async function removeAudienceSegments(mailbox, folder, contactEmail, segments) {
    const flags = segments.map((s) => `${AUDIENCE_PREFIX}${s}`);
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function removeContactMarkerSegments(mailbox, contactEmail, segments) {
...
export async function listAudienceSegments(mailbox, folder) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const isSent = folder !== "INBOX";
        ...
        try {
            ...
            for await (const msg of client.fetch("1:*", { flags: true, envelope: true })) {
                ...
                for (const flag of audienceFlags) {
                    ...
                    if (!segmentContacts.has(name))
                        segmentContacts.set(name, new Set());
...
async function ensureContactsFolder(client) {
    try {
        const folders = await client.list();
        if (folders.some((f) => f.path === CONTACTS_FOLDER))
            return;
...
export async function upsertContactMarker(mailbox, contact, segments) {
    const flags = segments.map((s) => `${AUDIENCE_PREFIX}${s}`);
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
function buildContactMarkerRfc822(mailbox, contact) {
    const lines = [
...
export async function listContactMarkers(mailbox) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        await ensureContactsFolder(client);
...
export async function getContactMetadataByEmails(mailbox, emails) {
...
export async function listAudienceSegmentsWithContacts(mailbox, folder) {
        ...
        for (const marker of markers) {
            for (const segName of marker.segments) {
                if (!segmentMap.has(segName))
                    segmentMap.set(segName, new Set());
...
export async function fetchUnclassifiedEmails(mailbox, folder, limit) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
        try {
            const result = await client.search({ unKeyword: "classified" }, { uid: true });
...
export async function fetchEmailByUid(mailbox, folder, uid) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function fetchEmailRawByUid(mailbox, folder, uid) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function deleteEmail(mailbox, folder, uid) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function findDraftsFolder(client, host) {
    try {
        const folders = await client.list();
...
export async function resolveDraftsFolder(mailbox) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function fetchDrafts(mailbox, limit, page = 1) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
...
export async function saveDraft(mailbox, raw) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
...
export async function deleteDraft(mailbox, uid) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
...
export async function fetchAttachmentByUid(mailbox, folder, uid, attachmentIndex) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
...
export async function saveCampaignConfig(mailbox, config) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
...
export async function loadCampaignConfig(mailbox, name) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
        ...
        try {
            ...
            for await (const msg of client.fetch("1:*", {
                ...
                if (msg.envelope.subject === `${CAMPAIGN_SUBJECT_PREFIX}${name}]`) {
                    if (!msg.source)
                        throw new Error(`Campaign "${name}" has no content`);
                    const parsed = await simpleParser(msg.source);
                    return JSON.parse(parsed.text || "{}");
                }
            }
...
export async function listCampaignConfigs(mailbox) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
...
export async function deleteCampaignConfig(mailbox, name) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const draftsFolder = await findDraftsFolder(client, mailbox.imapHost);
...
export async function getEmailHeaders(mailbox, folder, uid) {
    const client = createImapClient(mailbox);
    try {
        await client.connect();
        const lock = await client.getMailboxLock(folder);
