...
export function registerDraftsCommand(program: Command) {
  ...
  drafts
    ...
    .action(async (email: string) => {
      ...
      if (!handler) throw new Error('list_drafts tool not available');
  ...
  drafts
    ...
    .action(async (email: string, opts: { to: string; subject: string; text?: string; html?: string }) => {
      const handler = functionMap['create_draft'];
      if (!handler) throw new Error('create_draft tool not available');
  ...
  drafts
    ...
    .action(async (email: string, uid: string) => {
      const handler = functionMap['send_draft'];
      if (!handler) throw new Error('send_draft tool not available');
  ...
  drafts
    ...
    .action(async (email: string, uid: string) => {
      ...
      if (!handler) throw new Error('delete_draft tool not available');
      const result = await handler({ email, uid: parseInt(uid, 10) });
      const data = parseResult(result);
      output(program, data);
    });
