...
export function registerEmailsCommand(program: Command) {
  ...
  emails
    ...
    .option('--scan-limit <n>', 'Max messages to scan per folder', '500')
    .option('--limit <n>', 'Threads per page', '50')
    .option('--page <n>', 'Page number', '1')
    ...
    .action(async (email: string, opts: { folders: string; scanLimit: string; limit: string; page: string; messages?: boolean; subjectFallback: boolean }) => {
