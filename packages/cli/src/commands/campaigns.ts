...
export function registerCampaignsCommand(program: Command) {
  ...
  campaigns
    ...
    .action(async (email: string) => {
      ...
      if (!handler) throw new Error('list_campaigns tool not available');
  ...
  campaigns
    ...
    .action(async (email: string, name: string) => {
      ...
      if (!handler) throw new Error('get_campaign tool not available');
  ...
  campaigns
    ...
    .action(async (email: string, name: string) => {
      ...
      if (!handler) throw new Error('start_campaign tool not available');
      const result = await handler({ email, campaign: name });
      const data = parseResult(result);
      output(program, data);
    });
  ...
  campaigns
    ...
    .action(async (email: string, name: string) => {
      ...
      if (!handler) throw new Error('get_campaign_analytics tool not available');
      const result = await handler({ email, campaign: name });
      const data = parseResult(result);
      output(program, data);
    });
  ...
  campaigns
    ...
    .action(async (email: string, name: string) => {
      ...
      if (!handler) throw new Error('delete_campaign tool not available');
      const result = await handler({ email, campaign: name });
      const data = parseResult(result);
      output(program, data);
    });
