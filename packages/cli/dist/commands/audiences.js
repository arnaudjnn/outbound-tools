...
export function registerAudiencesCommand(program) {
    ...
    audiences
        ...
        if (!handler)
            throw new Error('list_audiences tool not available');
    ...
    audiences
        ...
        if (!handler)
            throw new Error('add_to_audience tool not available');
        const params = {
            email,
            segments: opts.segments.split(',').map((s) => s.trim()),
        };
        if (opts.firstName)
            params.firstName = opts.firstName;
        if (opts.lastName)
            params.lastName = opts.lastName;
        if (opts.company)
    ...
    audiences
        ...
        if (!handler)
            throw new Error('remove_from_audience tool not available');
        const result = await handler({
            email,
            segments: opts.segments.split(',').map((s) => s.trim()),
        });
